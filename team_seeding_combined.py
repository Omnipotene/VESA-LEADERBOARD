...
print("VESA League - Team Seeding System (All Seasons: S4+S11+S12 - Alias Matching)")
print("="*70)

# Load player ratings from all seasons (with top lobby bonus)
...
DEFAULT_RATING = 80  # For players not found in database (closer to median)
...
for team in teams:
    ...
    # Assign automatic tier based on team rating (bell curve distribution)
    if team_rating >= 145:
        tier = 'S+'
        tier_desc = 'Elite Pro'
    elif team_rating >= 130:
        tier = 'S'
        tier_desc = 'Elite'
    elif team_rating >= 115:
        tier = 'A+'
        tier_desc = 'Advanced High'
    elif team_rating >= 100:
        tier = 'A'
        tier_desc = 'High Skill'
    elif team_rating >= 90:
        tier = 'B+'
...
for tier in tier_order:
    ...
    if count > 0:
        print(f"  Tier {tier:3}: {count:3} teams")
